...
class ColorPaletteModal extends React.Component {
  ...
  componentDidMount() {
    this.state.newColors.forEach((item) => {
      item.selected = false;
    });
  }
  handlePress() {
          ...
          renderItem={({ item, index }) => {
            return (
              <View style={styles.colorField}>
                <Text style={{ paddingTop: 5 }}>{item.colorName}</Text>
                <Switch
                  value={item.selected}
                  onValueChange={(value) => {
                    let newColorArray = this.state.newColors;
                    newColorArray[index].selected = value;
                    this.setState({ newColors: newColorArray });
                  }}
                />
              </View>
            );
          ...
          ListHeaderComponent={
            <View>
              ...
              <TextInput
                ...
                onChangeText={(text) => this.setState({ paletteName: text })}
